...
* **Prediction Functions**:
  * `run_prediction` uses a single model to generate predictions on the test data and saves the results to a CSV file, detailing patient IDs, predicted classes, and probabilities.
  * If you want to use ensemble predictions, use `ensemble_predictions` and `ensemble_predictions_weighted` for predictions by averaging the outputs of multiple models.
...
### 4. Prediction
In `predictions.py`, run `run_prediction` to obtain predictions from a single model on the test data. You can also use the `ensemble_predictions` function for predictions from multiple models, which averages the outputs of several models.
